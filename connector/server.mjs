...
async function capture(round, trace=false) {
  ...
  try {
    ...
    const teamsData=await page.evaluate((names)=>names.map(name=>{
      ...
      if (!exact) return {team_key:name,name,present:null,source_status:'Squadra non trovata'};
    ...
    const observed_at=new Date().toISOString(); return {schema_version:1,league,season:'2026-2027',competition_id:competition,round,observed_at,source:'authenticated_ui',source_url:currentUrl,expected_total:10,inserted:teamsData.filter(t=>t.present).length,teams:teamsData};
...
server.listen(port,()=>{
  ...
  capture(1,true).then(()=>console.info('fantacalcio_trace_complete')).catch(error=>console.error('fantacalcio_trace_failed',{code:error instanceof Error?error.message:String(error)}));
});
