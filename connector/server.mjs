...
function json(res,status,body,ts) {
  const raw=JSON.stringify(body);
  res.writeHead(status,{'content-type':'application/json','cache-control':'no-store','x-fm-signature':sign(ts,raw)});
  res.end(raw);
}
...
async function loginAndOpen(page,round){
  ...
  const actualRound=match?Number(match[1]):null;
  if (actualRound!==round) throw new Error('connector_round_unavailable');
...
async function getContext(round){
  ...
  try{
    ...
    const map=new Map();
    for(const x of findTeamObjects(competitionTeamsPayload)) if(!map.has(x.name)) map.set(x.name,x.id);
    const teamIds=teams.map(name=>({name,id:map.get(name)}));
    if(teamIds.some(t=>!Number.isInteger(t.id))) {
      console.error('connector_team_mapping_failed',{expected:teams,discovered:[...map.entries()]});
      throw new Error('connector_team_mapping_failed');
    }

    const captured=await firstReq.allHeaders();
    const headers={};
...
async function fetchLineups(round){
  ...
  try{
    return await Promise.all(ctx.teamIds.map(async team=>{
      ...
      let payload;
      try{payload=await r.json();}catch{throw new Error('connector_lineup_invalid_json');}
      return {team,dto:payload?.teamLineupDto??null};
    }));
  }finally{
    ctx.page.off('response',ctx.listener);
...
const server=http.createServer(async(req,res)=>{
  ...
  if(req.method!=='POST'||req.url!=='/sync') return json(res,404,{error:'not_found'},String(Date.now()));
  const ts=String(req.headers['x-fm-timestamp']||Date.now());
  if(!auth(req,raw)){
    console.error('hmac_rejected',{secretFingerprint,hasSecret:Boolean(secret),timestampPresent:Boolean(req.headers['x-fm-timestamp']),signaturePresent:Boolean(req.headers['x-fm-signature'])});
    return json(res,401,{error:'unauthorized'},ts);
  }
  try{
    const round=Number(JSON.parse(raw).round);
    if(!Number.isInteger(round)||round<1||round>35) throw new Error('invalid_round');
    return json(res,200,{snapshot:await capture(round)},ts);
  }catch(error){
    const code=error instanceof Error?error.message:'connector_failed';
    console.error('connector_failed',{code,secretFingerprint});
    return json(res,code==='connector_auth_failed'?401:502,{error:code},ts);
  }
});
