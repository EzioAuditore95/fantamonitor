...
async function capture(round) {
  ...
  try {
    ...
    for (const item of discovered) if (!uniqueByName.has(item.name)) uniqueByName.set(item.name,item.id);
    const teamIds=teams.map(name=>({name,id:uniqueByName.get(name)}));
    if (teamIds.some(t=>!Number.isInteger(t.id))) {
      console.error('connector_team_mapping_failed',{expected:teams,discovered:[...uniqueByName.entries()]});
...
server.listen(port,()=>{
  ...
  capture(1).then(snapshot=>console.info('fantacalcio_startup_probe '+JSON.stringify({round:snapshot.round,inserted:snapshot.inserted,teams:snapshot.teams}))).catch(error=>console.error('fantacalcio_startup_probe_failed',{code:error instanceof Error?error.message:String(error)}));
});
