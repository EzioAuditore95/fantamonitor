...
async function loginAndOpen(page,round){
  ...
  const match=currentUrl.match(/\/manage-lineups\/(\d+)/i);
...
async function getContext(round){
  ...
  const listener=async response=>{
    if(/\/onboarding\/v1\/league\/competition\/teams/.test(response.url())&&response.status()===200){try{competitionTeamsPayload=await response.json();}catch{}}
  };
  ...
  try{
    ...
    const {baseUrl}=await loginAndOpen(page,round);const firstReq=await reqPromise;
...
async function fetchLineups(round){
  ...
  try{return await Promise.all(ctx.teamIds.map(async team=>{
    ...
    let payload;try{payload=await r.json();}catch{throw new Error('connector_lineup_invalid_json');}
...
async function capture(round){
  ...
  const started=Date.now();const items=await fetchLineups(round);const teamsData=items.map(x=>toTeamStatus(x,round));
...
async function verifyGithubAction(req){
  ...
  const {payload}=await jwtVerify(token,githubJwks,{issuer:'https://token.actions.githubusercontent.com',audience:'fantamonitor-auto-sync'});
  if(payload.repository!=='EzioAuditore95/fantamonitor'||payload.ref!=='refs/heads/main')throw new Error('oidc_scope');
  if(typeof payload.workflow_ref!=='string'||!payload.workflow_ref.includes('/.github/workflows/auto-sync.yml@refs/heads/main'))throw new Error('oidc_workflow');
  return payload;
}
async function rpc(name,args){
  if(!supabaseUrl||!supabaseKey||!autoSyncSecret)throw new Error('auto_sync_not_configured');
  const response=await fetch(`${supabaseUrl.replace(/\/$/,'')}/rest/v1/rpc/${name}`,{method:'POST',headers:{apikey:supabaseKey,'content-type':'application/json'},body:JSON.stringify(args),signal:AbortSignal.timeout(10000)});
  const text=await response.text();
  if(!response.ok)throw new Error(`supabase_${name}_${response.status}:${text.slice(0,180)}`);
...
const server=http.createServer(async(req,res)=>{
  ...
  if(req.method==='POST'&&req.url==='/sync'){
    ...
    try{const round=Number(JSON.parse(raw).round);if(!Number.isInteger(round)||round<1||round>35)throw new Error('invalid_round');return signedJson(res,200,{snapshot:await capture(round)},ts);}
    catch(error){const code=error instanceof Error?error.message:'connector_failed';console.error('connector_failed',{code,secretFingerprint});return signedJson(res,code==='connector_auth_failed'?401:502,{error:code},ts);}
  }
  if(req.method==='POST'&&req.url==='/auto-sync'){
    try{await verifyGithubAction(req);return plainJson(res,200,await runAutoSync());}
    catch(error){const code=error instanceof Error?error.message:'auto_sync_failed';console.error('auto_sync_failed',{code});return plainJson(res,code.startsWith('oidc_')?401:502,{error:code});}
  }
