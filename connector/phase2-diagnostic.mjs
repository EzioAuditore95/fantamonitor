...
const seen=[];let calendar=null,standings=null;
page.on('response',async r=>{const u=r.url();if(!u.includes('fantacalcio.it')||r.status()!==200)return;const ct=(await r.allHeaders())['content-type']||'';if(!ct.includes('json'))return;try{const j=await r.json();seen.push({url:u,shape:shape(j)});if(/\/league\/competition\/calendar\//.test(u))calendar=j;if(/standing|classifica/i.test(u))standings=j;}catch{}});
try{
 ...
 await page.goto(`${root}/standings`,{waitUntil:'networkidle',timeout:30000});
 ...
 if(Array.isArray(calendar)&&calendar.length){console.log('PHASE2_CALENDAR_SAMPLE',JSON.stringify({round:calendar[0],last:calendar[calendar.length-1]}));}
 if(standings)console.log('PHASE2_STANDINGS_SAMPLE',JSON.stringify(standings).slice(0,12000));
