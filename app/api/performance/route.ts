...
export async function GET(){
  ...
  if(!endpoint||!secret())return Response.json({error:'Connettore non configurato.'},{status:503,headers});
  try{
    ...
    if(!payload.competition||typeof payload.competition!=='object')throw new Error('competition_payload_invalid');
    return Response.json(payload.competition,{headers});
  }catch(error){console.error('performance_fetch_failed',error instanceof Error?error.message:error);return Response.json({error:'Dati competitivi non disponibili dal connettore Fantacalcio.'},{status:502,headers});}
}
