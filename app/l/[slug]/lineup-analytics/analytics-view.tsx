...
export default function AnalyticsView({config}:{config:LeagueConfig}){
  ...
  useEffect(()=>{let active=true;(async()=>{try{const [a,p]=await Promise.all([fetch(`/api/archive?league=${encodeURIComponent(cfg.slug)}`,{cache:'no-store'}),fetch(`/api/performance?league=${encodeURIComponent(cfg.slug)}`,{cache:'no-store'})]);const ad=await a.json() as Archive&{error?:string};const pd=await p.json() as PerformancePayload&{error?:string};if(!a.ok)throw new Error(ad.error||'Archivio non disponibile.');if(active){setArchive(ad);if(p.ok)setPerformance(pd);}}catch(e){if(active)setError(e instanceof Error?e.message:'Analisi non disponibili.');}finally{if(active)setLoading(false);}})();return()=>{active=false};},[cfg.slug]);
