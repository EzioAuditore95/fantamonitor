...
export default function PenaltiesPanel({config,data,round,onSaved}:{config:LeagueConfig;data:Archive|null;round:number;onSaved:()=>Promise<void>}){
  ...
  return <>
    ...
    {!data?<section className="panel blank-panel">Registro non ancora disponibile.</section>:<>
      ...
      <div className="coverage-note"><Info size={18}/><p><strong>{sum('verified')} esiti verificati su {sum('total')} squadra-giornate del periodo.</strong> I totali si basano sul registro verificato; le giornate senza esito, incluse quelle future, non consumano gettoni.</p></div>
    ...
    <Sheet open={!!detail} onOpenChange={v=>{if(!v)setDetail(null)}}><SheetContent className="drawer"><SheetHeader className="p-0"><SheetTitle>{detail}</SheetTitle><SheetDescription>Gettoni, penalità e registro delle verifiche.</SheetDescription></SheetHeader>{teamRow&&<><div className="team-penalty-total"><span>Penalità · {period}</span><strong>{euro(teamRow.penalty)}</strong></div>{selectedPeriods.map(p=><section key={p.key}><h3>{p.label} · {teamRow.periods[p.key].remaining?'1 GETTONE RESIDUO':'GETTONE UTILIZZATO'}</h3>{!teamRow.periods[p.key].entries.length?<p className="muted">Nessuna mancata consegna verificata.</p>:teamRow.periods[p.key].entries.map(e=><div className="timeline-row" key={e.id}><div><strong>Giornata {e.round} · Serie A {serieARound(cfg,e.round)}</strong><p>{e.token?'Gettone gratuito utilizzato':'Mancata consegna successiva'}</p><button className="detail-button" disabled={!data?.canManage} onClick={()=>edit(e.team,e.round)}>Rivedi esito</button></div><strong>{euro(e.charge)}</strong></div>)}</section>)}<button className="btn primary" disabled={!data?.canManage} onClick={()=>edit(teamRow.team,round)}><FileCheck2/>Verifica una giornata</button><h3><History size={16}/> STORICO DELLE VERIFICHE</h3>{reviews.filter(r=>r.team===detail&&(period===AGGREGATE_PERIOD||periodForRound(cfg,r.round).key===period)).slice().reverse().map(r=><div className="event-box review-event" key={r.id}><strong>Giornata {r.round} · {statusLabel[r.status]}</strong><p>{displayDate(r.recorded_at)} · revisione {r.revision}{latest.get(reviewKey(r.team,r.round))?.id!==r.id?' · superata':''}</p><p>{r.note}</p><p>Scadenza: {r.deadline?displayDate(r.deadline):'non registrata'}</p><a href={r.source_url} target="_blank" rel="noreferrer">Consulta la fonte</a></div>)}</>}</SheetContent></Sheet>
